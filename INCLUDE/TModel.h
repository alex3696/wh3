...
protected:
	...
	{
		// отключаем уведомление родителей, чтоб несколько раз не уведомл€ть
		// на Clear, Insert, а потом после обновлени€ каждого чилда
		auto enableParentNotify = (mOption & ModelOption::EnableParentNotify);
		mOption = mOption & ~ModelOption::EnableParentNotify;

		Clear(); // очищаем, если у детишек есть клиенты, то они отвал€тс€
		// загружаем детишек
		wxString query;
		...
		if (mVec && mVec->size() && (mOption & ModelOption::CascadeLoad))
			for (auto item : *mVec)
				item->LoadChilds();
		if (enableParentNotify)// включаем уведомлени€ родител€ как было
			mOption = mOption | ModelOption::EnableParentNotify;
		// 3  аскадно уведомить родителей об изменении
		DoNotifyParent();
