...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ViewToolbarAct::ViewToolbarAct(wxWindow* parent)
{
	...
	tool_bar->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewToolbarAct::OnCmd_Move, this, wxID_REPLACE);
	tool_bar->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewToolbarAct::OnCmd_Act, this, wxID_EXECUTE);
	tool_bar->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewToolbarAct::OnCmd_Redo, this, wxID_UNDO);
...
//-----------------------------------------------------------------------------
//virtual 
void ViewToolbarAct::SetObj(const rec::ObjInfo& rt)
{
}
//-----------------------------------------------------------------------------

void ViewToolbarAct::OnCmd_Move(wxCommandEvent& evt)
{
	sigMove();
}
//-----------------------------------------------------------------------------

void ViewToolbarAct::OnCmd_Act(wxCommandEvent& evt)
{
	sigAct();
}//-----------------------------------------------------------------------------

void ViewToolbarAct::OnCmd_Redo(wxCommandEvent& evt)
{
	sigRedo();
}
...
ViewPageDetail::ViewPageDetail(std::shared_ptr<IViewWindow> parent)
{
	...
	mViewToolbarAct = std::make_shared<ViewToolbarAct>(panel);
	panel->mAuiMgr.AddPane(mViewToolbarAct->GetWnd(), wxAuiPaneInfo().
		Name(wxT("ActToolBar"))
		.CaptionVisible(false)
	...
	auto toolbar_act = std::dynamic_pointer_cast<ViewToolbarAct>(mViewToolbarAct).get();
	panel->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewToolbarAct::OnCmd_Move, toolbar_act, wxID_REPLACE);
	panel->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewToolbarAct::OnCmd_Act, toolbar_act, wxID_EXECUTE);
	panel->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewToolbarAct::OnCmd_Redo, toolbar_act, wxID_UNDO);
...
//-----------------------------------------------------------------------------
//virtual 
std::shared_ptr<IViewToolbarAct>	ViewPageDetail::GetViewToolbarAct()const //override;
{
	return mViewToolbarAct;
}
//-----------------------------------------------------------------------------
//virtual 
