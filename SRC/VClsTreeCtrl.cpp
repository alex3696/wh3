...
VClsTreeCtrl::VClsTreeCtrl(wxWindow* parent,
							wxWindowID id,
							const wxPoint& pos,
							const wxSize& size,
							long style,
	...
	: TViewCtrlPanel(parent, id, pos, size, style, name)
{
	...
	Bind(wxEVT_MENU, &VClsTreeCtrl::OnBack, this, wxID_BACKWARD);
	Bind(wxEVT_COMMAND_DATAVIEW_ITEM_ACTIVATED, &VClsTreeCtrl::OnActivated, this);

}//
//-----------------------------------------------------------------------------
void VClsTreeCtrl::BuildGoUpItem()
{
...
//-----------------------------------------------------------------------------
void VClsTreeCtrl::SetModel(std::shared_ptr<IModel> model)
{
	mClsNode = std::dynamic_pointer_cast<MClsNode>(model);
	if (mClsNode)
	{
...
//---------------------------------------------------------------------------
void VClsTreeCtrl::OnActivated(wxDataViewEvent& evt)
{
	wxDataViewItem selectedItem = evt.GetItem();

	auto clsNodeArray = std::dynamic_pointer_cast<MClsNodeArray>(mVecModel);
	...
	if (clsNodeArray && mClsNode && selectedItem.IsOk())
	{
		...
		auto item = std::dynamic_pointer_cast<MClsNode>(childItem);
		if (item)
		{
...
//---------------------------------------------------------------------------
void VClsTreeCtrl::OnBack(wxCommandEvent& event)
{
	if (mClsNode)
	{
