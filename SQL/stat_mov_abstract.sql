...
DROP FUNCTION IF EXISTS pg_temp.stat_mov_abstract() CASCADE;
CREATE OR REPLACE FUNCTION pg_temp.stat_mov_abstract() 
 RETURNS VOID
AS $BODY$ 
DECLARE
...
BEGIN 
  ...
  FOR mov_row IN (SELECT ss.*,obj_name.title AS dst_otitle FROM _sum_dst_time_cls ss
    ...
    SELECT oqty INTO curr_oqty FROM pg_temp.stat_mov WHERE pg_temp.stat_mov.cid= mov_row.cid;
...
select * from pg_temp.stat_mov_abstract();
